...
class playManifestAction extends kalturaAction
{
	 ...
	 * @return array
	 */
	private function buildHttpFlavorsArray($oneOnly = false)
	{
		$this->buildHttpFlavorAssetArray($oneOnly);
	...
	{	
		...
		if(!$partner->getStorageServePriority() || 
			...
			$partner->getStorageServePriority() == StorageProfile::STORAGE_SERVE_PRIORITY_KALTURA_ONLY)
		{
			$localFlavors = $this->getLocalFlavors($this->flavorAssets);
		...
		$this->flavorAssets = $this->getLocalFlavors($this->flavorAssets);
	}
	...
	{
		$this->buildRtmpFlavorAssetArray();
	...
	private function serveHttp()
	{
		...
		$flavors = $this->buildHttpFlavorsArray(true);
